...
export const editUserFetch = (editUser) => async (dispatch) => {
	const response = await fetch(`http://127.0.0.1:3001/user/edit`, {
		method: "POST",
		headers: {
...
export const addOrderFromExecutorThunk = (id) => async (dispatch, setState) => {
	if (id) {
		fetch("http://127.0.0.1:3001/api/executor/order", {
			method: "POST",
			headers: {
				"Content-Type": "application/json",
			},
...
export const changeOrderStatusInWork = (id) => (dispatch, setState) => {
	fetch(`http://127.0.0.1:3001/api/orders/inwork/${id}`, {
		method: "PATCH",
		headers: {
			"Content-Type": "application/json",
		},
...
export const changeOrderCustomerStatusRequested = (id) => (
	...
	fetch(`http://127.0.0.1:3001/api/orders/requestedChange/${id}`, {
		method: "PATCH",
		headers: {
			"Content-Type": "application/json",
		},
...
export const deleteFetchDogAC = (id) => async (dispatch) => {
	// console.log(id);
	const response = await fetch(`http://127.0.0.1:3001/api/v1/dog/${id}`, {
		method: "DELETE",
		credentials: "include",
	});
...
export const uploadAvatarFetch = (formData) => async (dispatch) => {
	try {
		...
		const response = await fetch(`http://127.0.0.1:3001/user/avatar`, {
			method: "POST",
			credentials: "include",
			body: formData,
		});
...
export const closeOrderCustomer = (id) => (dispatch, setState) => {
	fetch(`http://127.0.0.1:3001/api/orders/closed/${id}`, {
		method: "PATCH",
		headers: {
			"Content-Type": "application/json",
		},
